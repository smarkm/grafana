...
func (hs *HTTPServer) LoginPostWithOTP(c *models.ReqContext, cmd dtos.LoginCommand) response.Response {
	authModule := ""
	...
	if cmd.OTP != "" {
		...
		if !exist {
			resp = response.Error(401, "Error while signing in user", nil)
			return resp
		}
		if otp.OTP != cmd.OTP {
			resp = response.Error(401, "Invalid OTP", nil)
			return resp

		}
		if otp.ExpireAt.Before(time.Now()) {
			resp = response.Error(401, "OTP expired", nil)
	...
	if err2 != nil {
		log.Error("Failed to get user", "error", err2)
	} else {
	...
	if cmd.OTP == "" {
		...
		if user.Email == "" {
			...
			if !sendOTP(hs, user, code, otp) {
				resp = response.Error(500, "Failed send OTP to user", err)
				return resp
			}
		}

		resp = response.Success("/public/otp.html?code=" + code)
		return resp
	}
	user = authQuery.User
